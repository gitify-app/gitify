...
describe('main/menu.ts', () => {
  ...
  describe('platform-specific accelerators', () => {
    // Helper to build template with mocked platform
    const buildTemplateWithPlatform = (isMac: boolean) => {
      // Mock the platform module
      vi.doMock('../shared/platform', () => ({ isMacOS: () => isMac }));

      // Clear prior captured calls
      (Menu.buildFromTemplate as vi.Mock).mockClear();

      // Create a new MenuBuilder instance and build menu
    ...
    it.todo('uses non-mac accelerator for toggleDevTools otherwise', () => {
