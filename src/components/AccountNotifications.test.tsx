...
describe("components/AccountNotifications.tsx", () => {
	...
	it("should open profile when clicked", async () => {
		const mockAccountProfileURL = jest
			.spyOn(links, "accountProfileURL")
			.mockImplementation();
		...
		expect(mockAccountProfileURL).toHaveBeenCalledTimes(1);
		expect(mockAccountProfileURL).toHaveBeenCalledWith(mockGitHubCloudAccount);
	});
