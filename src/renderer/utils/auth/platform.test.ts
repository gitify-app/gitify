...
describe('renderer/utils/auth/platform.ts', () => {
  describe('getPlatformFromHostname', () => {
    it('should return GitHub Cloud', () => {
      expect(getPlatformFromHostname('github.com' as Hostname)).toBe('GitHub Cloud');
      expect(getPlatformFromHostname('api.github.com' as Hostname)).toBe('GitHub Cloud');
    });
  ...
  describe('isCloudDataResidencyHost', () => {
    it('should return true for ghe.com hosts', () => {
      ...
      expect(isCloudDataResidencyHost('acme-corp.ghe.com' as Hostname)).toBe(true);
    });

    it('should return false for non ghe.com hosts', () => {
      expect(isCloudDataResidencyHost('github.com' as Hostname)).toBe(false);
      expect(isCloudDataResidencyHost('api.github.com' as Hostname)).toBe(false);
      expect(isCloudDataResidencyHost('github.gitify.app' as Hostname)).toBe(false);
    });
  });

  describe('isEnterpriseServerHost', () => {
    it('should return true for enterprise server host', () => {
      expect(isEnterpriseServerHost('github.gitify.app' as Hostname)).toBe(true);
      expect(isEnterpriseServerHost('api.github.gitify.app' as Hostname)).toBe(true);
    });
