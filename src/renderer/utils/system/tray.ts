 ...
 */
export function setTrayIconColorAndTitle(unreadNotifications: number, isOnline: boolean) {
  const settings = useSettingsStore.getState();

  let title = '';
  if (isOnline && settings.showNotificationsCountInTray && unreadNotifications > 0) {
